...
int campose( char *msgbuf, robot *myrobot) {
  string command;
  long session_id = -1;
  string p(msgbuf);
  ...
  if(command != CMD_CAMPOSE){
    //cout << "This aint campose!!!" << endl;
    return STATE_IDLE;
  }
...
int proc_cmd( char *msgbuf, robot *myrobot) {
  ...
  }else if(command == CMD_CAMPOSE){
    return STATE_CAMPOSE;
  ...
  }else if(command == CMD_AUCTION_INIT){
    return STATE_AUCTION_INIT;

  }else if(command == CMD_AUCTION_WON){
    return STATE_AUCTION_WON;

  }else if(command == CMD_AUCTION_BID){
    return STATE_AUCTION_BID;

  }else if(command == CMD_AUCTION_COMPLETE){
    return STATE_AUCTION_COMPLETE;
...
int client_ident( char *msgbuf, robot *myrobot) {
  ...
  //cout << "In IDENT" << endl;
  push_paint("STATUS", "PROCESSING IDENT", myrobot->get_session_id());
...
int auction_init( char *msgbuf, robot *myrobot) {
...
int auction_won( char *msgbuf, robot *myrobot) {
  string command;
  long session_id = -1;
  string p(msgbuf);
  stringstream istream(p);
...
int auction_bid( char *msgbuf, robot *myrobot) { return 0; }

int auction_complete( char *msgbuf, robot *myrobot) { return 0;}
...
int client_send_pose( char *msgbuf, robot *myrobot) {
  ...
  if(session_id < 0) {
    ...
    for (i = robots.begin(); i != robots.end(); ++i) {
      if ((*i)->get_type_id() != SID_GUI && (*i)->get_type_id() != SID_OHCAMERA && (*i)->get_type_id() != SID_COORDINATOR ) {
          num_poses++;
          posestream << " ";
    ...
    oss << num_poses << posestream.str();
	     ...
	     ((*i)->get_type_id() != SID_GUI && (*i)->get_type_id() != SID_OHCAMERA && (*i)->get_type_id() != SID_COORDINATOR )) {
      oss << "1";
      oss << " ";
      session_id = (*i)->get_session_id();
      oss << session_id;
...
int client_ask_pose( char *msgbuf, robot *myrobot) {
  ...
  if(myrobot->get_type_id() != SID_GUI && myrobot->get_type_id() != SID_OHCAMERA && myrobot->get_type_id() != SID_COORDINATOR ) {
      if (send_or_push(msgbuf, myrobot, session_id)) {
	  return STATE_IDLE;
      } else {
	  return STATE_QUIT;
...
void *client_handler( void *arg ) {
  ...
  while(!fDone){
	...
	case STATE_GOTO:
	    ...
	    break;
        case STATE_CAMPOSE:
            command = STATE_CAMPOSE; 
            break;
        case STATE_AUCTION_INIT:
            command = STATE_AUCTION_INIT; 
            break;
        case STATE_AUCTION_WON:
            command = STATE_AUCTION_WON; 
            break;
        case STATE_AUCTION_BID:
            command = STATE_AUCTION_BID; 
            break;
    case STATE_AUCTION_COMPLETE:
            command = STATE_AUCTION_COMPLETE; 
            break;
	default:
...
int main( int argc, char *argv[] ) {
  ...
  function_array[STATE_CAMPOSE] = &campose;
  function_array[STATE_AUCTION_INIT] = &auction_init; 
  function_array[STATE_AUCTION_WON] = &auction_won; 
  function_array[STATE_AUCTION_BID] = &auction_bid;
  function_array[STATE_AUCTION_COMPLETE] = &auction_complete; 
